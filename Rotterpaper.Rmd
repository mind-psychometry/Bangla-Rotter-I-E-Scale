...
cor.hits <- as.data.frame(reshape2::melt(upper))
colnames(cor.hits) <- c("value")
corr.histogram <- ggplot(cor.hits,aes(x=value)) + 
  geom_histogram(binwidth=.06, col=I("black"),fill="#00A08799")+apatheme+
  labs( y="", x = "Inter-item correlation")+scale_y_continuous(limits = c(0, 50), expand = expansion(mult = c(0, 0.1)))+scale_x_continuous(limits = c(-.22,.65), breaks =c(-.22,0, .30, .50, .65))

ggsave("Figures/correlation_histogram.png", corr.histogram, width = 3, height = 6, dpi = 600, bg ="white")
...
omega.two <- omega(reduced.model.2F.5, nfactors=3, fm="wls",n.iter=1,p=.05,poly=FALSE,key=NULL,  flip=TRUE,digits=2, title="Omega",sl=TRUE,labels=NULL,plot=TRUE,n.obs=NA,rotate="oblimin",Phi=NULL,option="equal",covar=FALSE)
```
...
Table <-  cbind( gt.table, gt.liket )%>% 
  ...
  gtExtras::gt_plt_dot(
    ...
    Items,
    palette = "ggthemes::fivethirtyeight") %>%  
  gtsave("Figures/descriptives_cfa.png",vwidth = 7000)
```
...
# ggsave("Figures/icc2.png",icc2, width = 8, height = 6, dpi = 1200, bg = "white")
