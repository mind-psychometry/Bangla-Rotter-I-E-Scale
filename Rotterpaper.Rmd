...
#Standard error
unidimensional_se <-coef(unidimensional_fit, IRTpars = TRUE,printSE = TRUE)
...
# mirt::plot(unidimensional_fit, type="infoSE")
...
ggsave("Figures/300/Figure4.jpg",personfit, width = 3.5, height = 3.5, dpi = 300, bg = "white")


```

```{r figfmisfit, fig.cap= "Distribution of the Zh statistic of 2PL model" }
knitr::include_graphics('Figures/300/Figure4.jpg', dpi =600)
```
...
```{r item-difficulty-cat, include =F}
#Categorize items based on discrimination and difficulty

##Discrimination
very_easy <-  sum(unidimensional_items$b< -2)
easy=  sum(-2<unidimensional_items$b &  unidimensional_items$b<= -0.50)
medium= sum(-0.51<unidimensional_items$b &  unidimensional_items$b<= 0.50)
hard= sum(.51<unidimensional_items$b &  unidimensional_items$b<= 2) 
very_hard =sum(unidimensional_items$b> 2)


##Difficulty
very_easy_items <- unidimensional_items[which(unidimensional_items$b< -2),]
easy_items <- unidimensional_items[which(-2<unidimensional_items$b &  unidimensional_items$b<= -0.50),]
medium_items <-unidimensional_items[which(-0.51<unidimensional_items$b &  unidimensional_items$b<= 0.50),]
hard_items <-unidimensional_items[which(.51<unidimensional_items$b &  unidimensional_items$b<= 2),]
very_hard_items <-unidimensional_items[which(unidimensional_items$b> 2),]
...
tic <- ggtestinfo_se(Rotter.one.IRT, unidimensional_fit)+ylim(0,4)
#Test-infor curve
tic <- ggtestinfo_se(Rotter.one.IRT, unidimensional_fit)+ylim(0,4)+apatheme+geom_area()+aes(fill =  "#E64B3599")+theme(legend.position = "none")+
...
```{r irt-iic,  include=F,warning=FALSE}
#Plot Item information curve
...
item.info.data.long <- reshape::melt(item.info.data)
...
ggsave("Figures/300/Figure6.jpg",ifc, width = 7, height = 8, dpi = 300, bg = "white")

```

```{r IRT-reliability, include=F}
#Estimate merginal reliability
...
ggreliability <- function(dataframe,model){
  ...
  p1 <-  ggplot2::ggplot(data, ggplot2::aes(x=Theta, y=reliability)) +ylim(0,1)+
   ...
   xlab(expression(theta))+ylab(expression(r(theta)[xx]))

  return(p1)
}


# r <- ggreliability(Rotter.one.IRT, unidimensional_fit)
reliability <- ggreliability(Rotter.one.IRT, unidimensional_fit)+apatheme+geom_area(fill="#69b3a2")+theme(legend.position = "none")+
scale_fill_identity()
```
...
```{r, include =F}
icctic <- cowplot::plot_grid(icc2, tic, reliability,
                   labels = c("(A)", "(B)", "(C)"),
                   ncol=1, 
                  align = "h", 
                  # label_fontfamily = "Arial",
...
ggsave("Figures/300/Figure5.jpg", width = 7, height = 9, dpi = 300, bg = "white")
```

```{r, include =F}
#plot the merginal relibility
...
```{r FigTIC, fig.cap= "Item Information Curves of Bangla Rotter’s I-E Scale. Item 18 carried the highest level of information across the theta continuum and item 04 carried the lowest information"}
  knitr::include_graphics('Figures/300/Figure6.jpg', dpi =600)
...
knitr::include_graphics('Figures/300/Figure5.jpg', dpi =600)
```
